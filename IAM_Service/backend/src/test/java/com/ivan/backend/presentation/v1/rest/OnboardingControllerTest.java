...
class OnboardingControllerTest {
        ...
        void should_return_409_when_user_already_exists() throws Exception {
                ...
                when(onboardingInputPort.execute(any()))
                                .thenThrow(new UserAlreadyExistsException("L'utilisateur existe déjà"));
        ...
        @Test
        @DisplayName("Onboarding : devrait retourner 500 pour une erreur technique inattendue")
        void shouldReturn500WhenUnexpectedErrorOccurs() throws Exception {
                // GIVEN
                OnboardingRequest request = new OnboardingRequest(
                                "Ivan", "Dev", "error@test.com", "Password123!", "Ma Super Entreprise");

                // On simule une RuntimeException (par ex: bug NullPointer ou DB down)
                when(onboardingInputPort.execute(any(OnboardingRequest.class)))
                                .thenThrow(new RuntimeException("Erreur serveur imprévue"));
